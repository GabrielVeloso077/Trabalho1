...
Personagem* Simulador::proximoPersonagem(vector<Personagem*>& equipe)
{
    ...
    vector<Personagem*> personagensDisponiveis;
    for (auto personagem : equipe)
    {
        if (personagem->getVida() > 0)
        {
            personagensDisponiveis.push_back(personagem);
        }
    }

    int tamanhoDisponiveis = personagensDisponiveis.size();
    if (tamanhoDisponiveis == 0)
    {
        return nullptr;
    }

    int indiceAleatorio = std::rand() % tamanhoDisponiveis;
...
void Simulador::iniciarSimulacao()
{
    ...
    {
        // Seleciona um personagem atacante aleatório da equipe 1
        Personagem* personagemAtacante = proximoPersonagem(equipe1);
        
        // Seleciona um personagem defensor aleatório da equipe 2
        Personagem* personagemDefesa = proximoPersonagem(equipe2);
        
        // Caso não haja personagens atacantes ou defensores disponíveis, encerra a simulação
        if (!personagemAtacante || !personagemDefesa) {
            break;
        }
        
        // Cria o combate entre os personagens selecionados
